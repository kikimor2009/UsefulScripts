...
while true ; do
  ...
  then
    while true ; do
      ...
      power=$(grep -o 'current: .*A' ampPower.txt | cut -d " " -f2  | cut -d '.' -f1)
      rm -f /home/bin/ampPower.txt
      if (( power < 3 )); then
        echo "[ $(date -R) ] : Not enough power. Power = ${power} A. Restart" | tee -a debug_out*
        echo restart > /home/bin/restart.txt
        sleep 3
        rm -f /home/bin/restart.txt
      else
        echo "[ $(date -R) ] : Power ok. Power = ${power} A." | tee -a debug_out*
